...
import tools.torch_DLT as torch_DLT
import tools.torch_homo_transform as torch_homo_transform
import tools.torch_tps_transform as torch_tps_transform
import ssl
...
def data_aug(img1, img2):
    ...
    white = torch.ones([img1.size()[0], img1.size()[2], img1.size()[3]]).cuda()  # shape=[1, 512, 512]
    random_colors = torch.randn(3).uniform_(0.7,1.3).cuda()
    color_image = torch.stack([white * random_colors[i] for i in range(3)], axis=1) # torch.stack([ [1, 512, 512], [1, 512, 512], [1, 512, 512] ], axis=1) ----> shape=[1, 3, 512, 512]
    img1_aug  *= color_image # shape=[1, 3, 512, 512]
...
def build_model(net, input1_tensor, input2_tensor, is_training = True):
    ...
    if is_training == True:
        ...
        H_motion, mesh_motion = net(aug_input1_tensor, aug_input2_tensor)  # 单应性矩阵， 网格移动矩阵
    else: # (1,8) , (1,338)==>(1,13*2**2)
        H_motion, mesh_motion = net(input1_tensor, input2_tensor)

    H_motion = H_motion.reshape(-1, 4, 2)                           #  单映性矩阵 --------》 torch.Size([1, 4, 2])
    mesh_motion = mesh_motion.reshape(-1, grid_h+1, grid_w+1, 2)    #  网格上每一个点的偏移量 --------》 torch.Size([1, 13, 13, 2])

    # initialize the source points bs x 4 x 2
    src_p = torch.tensor([[0., 0.], [img_w, 0.], [0., img_h], [img_w, img_h]])  # 原图像4个角的坐标  .shape=[4,2]
    if torch.cuda.is_available():
        src_p = src_p.cuda()
    src_p = src_p.unsqueeze(0).expand(batch_size, -1, -1) # .shape=[bs=1,4,2]
    # target points
    dst_p = src_p + H_motion # .shape=[1,4,2] + .shape[1,4,2] =====> 原始的4角顶点坐标 + 对应的偏移量(学习而来!!)  == > 仿射之后的点的坐标
    # solve homo using DLT
    H = torch_DLT.tensor_DLT(src_p, dst_p) # .shape = [1,3,3]
    ...
    rigid_mesh = get_rigid_mesh(batch_size, img_h, img_w)   # [1,13,13,2] ---> 原始网格（小正方形格子）
    ini_mesh = H2Mesh(H, rigid_mesh)      # [1,13,13,2] ---> 每个各自顶点坐标的偏移量
    mesh = ini_mesh + mesh_motion      # 已经变形的网格，每个顶点坐标的位置  [1,13,13,2]


    norm_rigid_mesh = get_norm_mesh(rigid_mesh, img_h, img_w) # 原始的经过 归一化的 小正方形格子   [1,169,2]
    norm_mesh = get_norm_mesh(mesh, img_h, img_w)             # 已经形变的网络 归一化的   [1,169,2]
    ...
    overlap = torch_tps_transform.transformer(warp_mesh_mask, norm_rigid_mesh, norm_mesh, (img_h, img_w)) #  torch.Size([1, 3, 512, 512])
    overlap = overlap.permute(0, 2, 3, 1).unfold(1, int(img_h/grid_h), int(img_h/grid_h)).unfold(2, int(img_w/grid_w), int(img_w/grid_w)) # torch.Size([1, 12, 12, 3, 42, 42])
    overlap = torch.mean(overlap.reshape(batch_size, grid_h, grid_w, -1), 3) # torch.Size([1, 12, 12])
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.regressNet1_part2 = nn.Sequential(   # ================> 单应性映射
        ...
        self.regressNet2_part2 = nn.Sequential( #  # ================> 网格
        ...
        ssl._create_default_https_context = ssl._create_unverified_context # 下载的时候不在验证服务器的SSL证书，让下载更快速/顺畅!!
    ...
    def forward(self, input1_tesnor, input2_tesnor):
        ...
        # input1_tesnor.shape = input2_tesnor.shape = [1,3,512,512]
        feature_1_64 = self.feature_extractor_stage1(input1_tesnor)  # [1,512,64,64]       /8
        feature_1_32 = self.feature_extractor_stage2(feature_1_64)   # [1,1024,32,32]----------------------------->   /16
        feature_2_64 = self.feature_extractor_stage1(input2_tesnor)  # [1,512,64,64]       /8
        feature_2_32 = self.feature_extractor_stage2(feature_2_64)   # [1,1024,32,32]----------------------------->   /16

        ######### stage 1  -----------> Global Transformation
        correlation_32 = self.CCL(feature_1_32, feature_2_32) # torch.Size([1, 2, 32, 32]) ===> 特征图上的图像上的偏移量!!!
        temp_1 = self.regressNet1_part1(correlation_32)  # temp_1.shape = (1, 256, 4, 4)
        temp_1 = temp_1.view(temp_1.size()[0], -1)       # temp_1.shape = torch.Size([1, 4096])
        offset_1 = self.regressNet1_part2(temp_1)        # offset_1.shape = torch.Size([1, 8])
        H_motion_1 = offset_1.reshape(-1, 4, 2)          # H_motion_1.shape = torch.Size([1, 4, 2])  ===> 对应论文的 4-pt motion
        ...
        return offset_1, offset_2  # offset_1.shape = [1,8],  offset_2.shape = torch.Size([1, 338])   ====> aqrt(338/2)=13


    def extract_patches(self, x, kernel=3, stride=1):
        if kernel != 1: # ------------------- x.shape = torch.Size([1, 1024, 32, 32])
            x = nn.ZeroPad2d(1)(x) # ------------------- x.shape = torch.Size([1, 1024, 34, 34])
        x = x.permute(0, 2, 3, 1)  # ------------------- x.shape = torch.Size([1, 34, 34, 1024])
        all_patches = x.unfold(1, kernel, stride).unfold(2, kernel, stride) # all_patches.shape = torch.Size([1, 32, 32, 1024, 3, 3])
        return all_patches


    def CCL(self, feature_1, feature_2):
        bs, c, h, w = feature_1.size()
        # feature_1.shape = feature_2.shape = [1, 1024, 32, 32]
        norm_feature_1 = F.normalize(feature_1, p=2, dim=1) # 在channel维度上做归一化, 相当于点 的特征，不同的特征图间要做归一化
        norm_feature_2 = F.normalize(feature_2, p=2, dim=1)
        #print(norm_feature_2.size())

        patches = self.extract_patches(norm_feature_2) # torch.Size([1, 32, 32, 1024, 3, 3])   ---->这种操作有一种莫名的震撼!!!
        if torch.cuda.is_available():
            patches = patches.cuda()
        #                                                    32*32=1024
        matching_filters  = patches.reshape((patches.size()[0], -1, patches.size()[3], patches.size()[4], patches.size()[5]))  # torch.Size([1, 1024, 1024, 3, 3])

        match_vol = []             #                                   输出通道数   输入通道数    卷积核-高   卷积核-宽
        for i in range(bs):        # .shape=[1,1024,32,32]        .shape=[1024,     1024,        3,         3])
            single_match = F.conv2d(norm_feature_1[i].unsqueeze(0), matching_filters[i], padding=1) #  torch.Size([1, 1024, 32, 32]) -你敢信这是计算2个特征图的的相似度概率的
            match_vol.append(single_match) # single_match.shape = torch.Size([1, 1024, 32, 32])

        match_vol = torch.cat(match_vol, 0)    # torch.Size([1, 1024, 32, 32])
        #print(match_vol .size())

        # scale softmax
        softmax_scale = 10
        match_vol = F.softmax(match_vol*softmax_scale,1)  # torch.Size([1, 1024, 32, 32])  # 2图像特征图，每一的区域1024个维度上，哪一个维度更像？

        channel = match_vol.size()[1] # 在 1024个维度上比较!!
        ...
        if torch.cuda.is_available():
            h_one = h_one.cuda()
            one1w = one1w.cuda()
        h_one = torch.matmul(h_one.unsqueeze(1), one1w)  # .shape=(h,w) = .shape(32,1) * .shape(1,32) =(32,32)
        h_one = h_one.unsqueeze(0).unsqueeze(0).expand(bs, channel, -1, -1) # torch.Size([1, 1024, 32, 32])
        ...
        w_one = w_one.unsqueeze(0).unsqueeze(0).expand(bs, channel, -1, -1) # torch.Size([1, 1024, 32, 32])
        # h_one 与 w_one 貌似是在做 图像网格化
        c_one = torch.linspace(0, channel-1, channel)
        if torch.cuda.is_available():
            c_one = c_one.cuda()
        c_one = c_one.unsqueeze(0).unsqueeze(2).unsqueeze(3).expand(bs, -1, h, w) # # torch.Size([1, 1024, 32, 32])

        flow_h = match_vol*(c_one//w - h_one)  # flow_h.shape = torch.Size([1, 4096, 64, 64])
        flow_h = torch.sum(flow_h, dim=1, keepdim=True) # # flow_h.shape = torch.Size([1, 1, 64, 64])
        ...
        return feature_flow # torch.Size([1, 2, 32, 32])
