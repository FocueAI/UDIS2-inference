...
def transformer(U, theta, out_size, **kwargs):
    ...
    def _transform(theta, input_dim, out_size):
        ...
        #  Changed
        theta = theta.reshape([-1, 3, 3]).float()  # 单应性矩阵 H  .shape=[1,3,3]

        out_height, out_width = out_size[0], out_size[1]  # 512, 512
        grid = _meshgrid(out_height, out_width)       # (3,512*512=262144)   其中的值为等差数列 -1 ~ 1
        grid = grid.unsqueeze(0).reshape([1,-1])      # .shape = torch.Size([1, 512*512*3=786432])
        shape = grid.size()
        grid = grid.expand(num_batch,shape[1])       # [1, 786432]
        grid = grid.reshape([num_batch, 3, -1])      # (1, 3,512*512=262144)

        T_g = torch.matmul(theta, grid)   # torch.Size([1, 3, 262144]) 含义: 网格做单应性变换后的新坐标
        x_s = T_g[:,0,:]  # torch.Size([1, 262144]) -----> 变换后 x的坐标
        y_s = T_g[:,1,:]  # torch.Size([1, 262144]) -----> 变换后 y的坐标
        t_s = T_g[:,2,:]  # 每个点的缩放因子

        t_s_flat = t_s.reshape([-1])   # torch.Size([262144])
